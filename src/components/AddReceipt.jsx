...
const addReceipt = async ({ date, accountId, amount, categoryId, image, currentBalance }) => {
    try {
...
const AddReceipt = () => {
    ...
    const saveReceipt = (event) => {
        ...
        addReceipt({
            ...
            image: receipt,
            currentBalance: accounts.find(account => `${account.id}` === `${accountId}`).currentBalance
        });
    };
    ...
    return (
        <div>
            <form>
                ...
                <button type="submit" onClick={saveReceipt}>Add</button>
            </form>
        </div>
    );
};
