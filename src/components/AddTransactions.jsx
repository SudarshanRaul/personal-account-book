...
const AddTransactions = () => {
    ...
    const createEmptyRecords = (num) => {
        const emptyRecords = Array(num).fill().map(() => ({
            date: '',
            amount: '',
            category: '',
            type: '',
    ...
    const updateTransaction = (index, field, value) => {
        ...
        if(field === 'amount') {
            value < 0 ? updatedTransactions[index]['type'] = 'Expense' : updatedTransactions[index]['type'] = 'Income';
        }
        ...
        <div className="txn-container">
            ...
            {transactions.map((txn, index) => (
                <div key={index} className="txn-row">
                    ...
                    <select value={txn.accountId} onChange={(e) => updateTransaction(index, "accountId", e.target.value)}>
                        {accounts && accounts.map(({ id, name }, index) => (
                            <option key={index} value={id}>{name}</option>
                        ))}
                    </select>
                </div>
